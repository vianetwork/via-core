...
impl ViaBridgeDal<'_, '_> {
    ...
    pub async fn insert_bridge_tx(
        ...
        sqlx::query!(
            r#"
            INSERT INTO
                via_bridge_tx (proof_reveal_tx_id, hash, data)
            VALUES
            ($1, $2, $3)
            "#,
            proof_reveal_tx_id,
    ...
    pub async fn list_bridge_txs_not_yet_processed(&mut self) -> DalResult<Vec<(Vec<u8>, i64)>> {
        let rows = sqlx::query!(
            r#"
            SELECT
                id,
        ...
        let bridge_txs = rows
            .into_iter()
            .map(|row| (row.proof_reveal_tx_id, row.id))
