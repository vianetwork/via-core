...
async fn update_compiler_versions(connection_pool: &ConnectionPool<Core>) {
    ...
    let via_home = workspace_dir_or_current_dir();

    let zksolc_path = via_home.join("etc/zksolc-bin/");
    ...
    let solc_path = via_home.join("etc/solc-bin/");
    ...
    let zkvyper_path = via_home.join("etc/zkvyper-bin/");
    ...
    let vyper_path = via_home.join("etc/vyper-bin/");
