...
pub enum ViaL2TxSubmissionResult {
    Added,
    Replaced,
    AlreadyExecuted,
    Duplicate,
...
impl fmt::Display for ViaL2TxSubmissionResult {
...
impl ViaTransactionsDal<'_, '_> {
    ...
    pub async fn insert_transaction_l2(
    ...
    ) -> DalResult<ViaL2TxSubmissionResult> {
        let tx_hash = tx.hash();
        ...
        if is_duplicate {
            ...
            return Ok(ViaL2TxSubmissionResult::Duplicate);
        }
        ...
        let l2_tx_insertion_result = match query_result {
            Ok(option_query_result) => match option_query_result {
                Some(true) => ViaL2TxSubmissionResult::Replaced,
                Some(false) => ViaL2TxSubmissionResult::Added,
                None => ViaL2TxSubmissionResult::AlreadyExecuted,
            },
            Err(err) => {
