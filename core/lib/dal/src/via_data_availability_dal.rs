...
impl ViaDataAvailabilityDal<'_, '_> {
    ...
    async fn get_proof_blob_id_with_blocknumber(
        &mut self,
        l1_batch_number: u32,
        ...
        .instrument("get_proof_blob_id_with_blocknumber")
        .with_arg("block_number", &l1_batch_number)
        .fetch_optional(self.storage)
        .await?;

        Ok(result.map(|row| row.blob_id))
    }

    async fn get_l1_batch_blob_id_with_blocknumber(
        &mut self,
        l1_batch_number: u32,
    ) -> DalResult<Option<String>> {
        let result = sqlx::query!(
        ...
        .instrument("get_l1_batch_blolb_id_with_blocknumber")
        .with_arg("block_number", &l1_batch_number)
        .fetch_optional(self.storage)
        .await?;
