...
impl TestTemplate {
    ...
    pub fn verifier_empty() -> anyhow::Result<Self> {
        let db_url = env::var("TEST_DATABASE_VERIFIER_URL").context(
            "TEST_DATABASE_VERIFIER_URL must be set. Normally, this is done by the 'zk' tool. \
            Make sure that you are running the tests with 'zk test rust' command or equivalent.",
        )?;
        Ok(Self(db_url.parse()?))
    }
...
impl<DB: DbMarker> ConnectionPool<DB> {
    ...
    pub async fn verifier_test_pool() -> ConnectionPool<DB> {
        const DEFAULT_CONNECTIONS: u32 = 100; // Expected to be enough for any unit test.
        Self::constrained_verifier_test_pool(DEFAULT_CONNECTIONS).await
    }
    ...
    pub async fn constrained_verifier_test_pool(connections: u32) -> ConnectionPool<DB> {
        assert!(connections > 0, "Number of connections must be positive");
        let mut builder = TestTemplate::verifier_empty()
            .expect("failed creating verifier test template")
            .create_db(connections)
            .await
            .expect("failed creating verifier database for tests");
        let mut pool = builder
            .set_acquire_timeout(Some(Self::TEST_ACQUIRE_TIMEOUT))
            .build()
            .await
