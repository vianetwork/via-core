...
impl IndexerMetaDal for SequencerStorage {
    async fn get_last_processed_l1_block(&mut self, module: &str) -> anyhow::Result<u32> {
        ...
        Ok(conn.via_indexer_dal().get_last_processed_l1_block(module).await? as u32)
    }

    async fn init_indexer_metadata(&mut self, module: &str, start_block: u32) -> anyhow::Result<()> {
        let mut conn = self.pool.connection_tagged("via_btc_watch").await?;
        Ok(conn.via_indexer_dal().init_indexer_metadata(module, start_block).await?)
    }

    async fn update_last_processed_l1_block(&mut self, module: &str, new_block: u32) -> anyhow::Result<()> {
        let mut conn = self.pool.connection_tagged("via_btc_watch").await?;
        Ok(conn.via_indexer_dal().update_last_processed_l1_block(module, new_block).await?)
    }
}
