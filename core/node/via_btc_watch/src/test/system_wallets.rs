...
mod tests {
    ...
    #[tokio::test]
    async fn test_system_contract_upgrade_persists_all_pairs() -> anyhow::Result<()> {
        let pool = ConnectionPool::<Core>::test_pool().await;
        let mut indexer = test_create_indexer();
        ...
        let expected_upgrade_tx = ViaProtocolUpgrade::default().create_protocol_upgrade_tx(
        ...
        let mut upgrade_processor =
            GovernanceUpgradesEventProcessor::new(btc_client, initial_version);
        ...
        let stored_upgrade_tx = storage
            .protocol_versions_dal()
            .get_protocol_upgrade_tx(proposal_input.version.minor)
            .await?
            .expect("activated upgrade transaction must be stored");
        assert_eq!(stored_upgrade_tx.execute, expected_upgrade_tx.execute);
        assert_eq!(
            stored_upgrade_tx.common_data,
            expected_upgrade_tx.common_data
        );
        Ok(())
    }
