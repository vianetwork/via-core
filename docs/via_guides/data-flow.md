...
publication to the data availability layer (Bitcoin). As explained in its
[README](../../core/node/via_da_dispatcher/README.md), it extends the functionality of the original zkSync da_dispatcher
but with specific adaptations for Bitcoin:
...
data onto the Bitcoin blockchain. This component:
...
Bitcoin's Taproot features to efficiently embed data. The inscription contains:
...
After batch data is committed to Bitcoin, the next stage in the data flow is generating cryptographic proofs that verify
...
available and verifiable. This crucial step in the data flow ensures the integrity and trustlessness of the L1 batch by
anchoring its cryptographic proof to Bitcoin's security.
...
As noted in the codebase, these "tasks" are implemented through database table entries rather than direct communication.
The dispatcher writes to specific tables, which are then queried by other components looking for work to perform. This
pattern follows the indirect communication principle established in the system architecture.
