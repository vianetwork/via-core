...
- `core/lib/dal` remains the main-node DAL; upstream squashed old migrations and added
  new ones through `20260602…` (airbender proofs, unsealed-batch index). Via's deleted
...
1. **Renumber all via migrations after the pin's latest migration** (>
   `20260602000000`), squashed into a small ordered set (one per subsystem:
...
3. Re-derive the 29 modified upstream DAL files against v29 — most touch
