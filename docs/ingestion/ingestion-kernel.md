...
- Status: IMPLEMENTED through the database adapter on `spike/kernel-base`: typed contract (wire format 3), positioned parser, production engine, generic Postgres adapter, and a 20-fixture acceptance suite passing against real Postgres for all three roles. Sections below marked as proposals were since ratified or revised; each carries its current status.
- Date: 2026-07-10
