...
- Status: DRAFT (design consolidation, no implementation started)
- Date: 2026-07-10
...
RPC, retries, SQL, polling, wallet import, metrics, and process supervision all stay outside, at explicit seams. The first implementation step is a refactor of the existing `BitcoinInscriptionIndexer`, not a new framework.
...
| Irrelevant to role | Valid event outside this node's projection | Record disposition; advance |
...
### 5.6 Intra-block ordering and protocol context (proposed rule)
...
2. Messages within a transaction are ordered by a parser-defined stable `message_ordinal` derived from their location in the transaction — never enum declaration order or map iteration order.
3. Every message in a transaction is validated against the protocol context as of the **start of that transaction**.
4. A valid context transition (wallet rotation, governance change) takes effect after its transaction; later transactions in the same block see the new context.
5. A transaction cannot authorize itself with a wallet update it contains.
6. Conflicting updates of the same role within one transaction are rejected unless the protocol explicitly defines their resolution.

This gives deterministic same-block activation without range-wide retroactivity. The whole block still commits or rolls back as one unit; there is no externally visible sub-block state. Conversely, multi-block atomicity is too coarse: if block N is valid and N+1 halts on a missing dependency, N stays committed and the checkpoint rests exactly at N.

### 5.7 Acceptance fixture suite (the judge)

The fixtures below are written before any implementation and are the acceptance contract for the deposit/tracked-output vertical slice (slice 4). If independent implementations are compared, this suite is the judge; divergence between implementations that both pass is read as an ambiguity in this spec, to be resolved here first. Fixtures assert logical visibility and state invariants, never a particular SQL statement order — independent implementations may organize writes differently. Each runs against all three real aggregate adapters where applicable, not only an in-memory fake. Consolidated from the F1-F5 sketch and the Codex fixture consultation (`research/codex-fixture-opinion.md`).
...
- **Slice 1: salvage.** Port the PR 379 keepers (section 4) as independent commits. No locator tables, no backfill. Independently valuable even if the kernel never happens.
- **Slice 2: extract envelope-to-plan.** Refactor `BitcoinInscriptionIndexer::process_block_inner` so fetching is outside and a shared function accepts a `BitcoinBlockEnvelope`. First outputs: raw Via candidate bytes, inclusions, bridge/tracked output creations, `DepositObserved` events, header block time. Existing production behavior stays active; compare old messages with the new plan in tests/shadow.
- **Slice 3: observation schema plus three aggregate adapters.** Same minimal observation/checkpoint schema in all three DB families (canonical migration source or checksum test against drift). Implement `apply_block` for the deposit event in all three. The three adapters mapping one event to different domain rows is precisely the test of whether the shared/adapter boundary is honest.
- **Slice 4: prove atomicity and reorg.** Implement until the full frozen fixture suite of section 5.7 passes against all three databases. The suite is written first (in `spike/kernel-base`, before any kernel implementation) and is the acceptance contract; if two implementations are compared, it is the judge.

  **Bake-off outcome (2026-07-10).** Two independent implementations of the deposit slice were built against the frozen contract and suite — `spike/kernel-claude` (Claude) and `spike/kernel-codex` (Codex, gpt-5.6-sol ultra), each with in-memory adapters carrying real durability semantics. Both pass all 15 fixtures; a cross-implementation harness then confirmed the two engines produce **byte-identical canonical plans** for identical envelopes — the deterministic-plan property validated across independent implementations. The exercise surfaced two spec defects, both fixed on `spike/kernel-base` before freezing: (#1) dependency discovery was unimplementable for a pure engine until made explicitly conservative; (#2) `message_ordinal` derivation was underspecified — two conforming engines hashed differently until the rule (lowest encoding output index) was pinned in the contract. Reference for the Postgres adapters (slice 3): Codex's implementation (richer edge handling — coverage manifests, empty-checkpoint context binding, durable halt on unknown ancestor), grafting the journal-undo revert from the Claude implementation in place of per-block snapshots (better growth profile). Reports: `research/codex-bakeoff-report.md`, `research/codex-bakeoff-blocked-round1.md`. Neither spike branch merges.
- **Slice 5: migrate event families.** Order: deposits/timestamps; tracked-output spends/withdrawals; local broadcast inclusion/confirmation; DA/proof reference chains; verifier attestations and governance; wallet rotations last, after their ordering rule is settled. Capability matrix row per family per role; production cursor never advances past an unimplemented family for that role.
...
2. **Wallet rotation effective ordering**: proposed answer in section 5.6 (transaction-boundary rule: effective after its containing transaction, within the same block). Needs ratification in the slice-0 ADR; regenesis is the cheap moment to change it if it differs from current accidental behavior.
...
3. **Standalone indexer**: implements the full `revert_to` contract, or is excluded from the pruned-compatibility claim.
