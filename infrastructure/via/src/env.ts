...
export function get(print: boolean = false) {
    ...
    const currentEnv = (process.env.VIA_ENV =
        process.env.VIA_ENV || inCurrent || (process.env.IN_DOCKER ? 'docker' : 'dev'));
...
export async function gitHooks() {
    if (fs.existsSync('.git')) {
        await utils.exec(`
            ...
            git config --local core.hooksPath ${process.env.VIA_HOME}/.githooks
        `);
    }
}
...
export function mergeInitToEnv() {
    const environment = process.env.VIA_ENV!;
